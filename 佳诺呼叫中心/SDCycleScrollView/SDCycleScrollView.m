...
+ (instancetype)cycleScrollViewWithFrame:(CGRect)frame imagesGroup:(NSArray *)imagesGroup where:(NSInteger)where
{
    SDCycleScrollView *cycleScrollView = [[self alloc] initWithFrame:frame];
    cycleScrollView.imagesGroup = [NSMutableArray arrayWithArray:imagesGroup];
    cycleScrollView.where = where;
    return cycleScrollView;
}

+ (instancetype)cycleScrollViewWithFrame:(CGRect)frame imageURLsGroup:(NSArray *)imageURLsGroup where:(NSInteger)where
{
    SDCycleScrollView *cycleScrollView = [[self alloc] initWithFrame:frame];
...
{
    NSString *url = self.imageURLsGroup[index];
    
    // 如果有缓存，直接加载缓存
    NSData *data = [NSData getDataCacheWithIdentifier:url];
    if (data) {
        ...
        [NSURLConnection sendAsynchronousRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:url]]
                               ...
                               completionHandler:^(NSURLResponse *response, NSData *data, NSError *connectionError){
                                   if (!connectionError) {
                                       ...
                                       [data saveDataCacheWithIdentifier:url];
                                   } else { // 加载数据失败
                                       static int repeat = 0;
